...
\end{equation}
```
Since the distribution we want to compare are multivariate ones, there
are not variety of procedures that can compare them directly. The idea
...
distribuion using two sample hypothesis test as Kolmogorov-Smirnov test.
...
is an IID sample from a single distribution F, and so the group labels
are meaningless.
...
The two sample hypothesis test can be turn to a goodness of fit test.
Here we have the assumptions that the sample $\underline{z}$ came from
an reference distribution (Say $F$). We can set up the following
hypothesis system:
...
\end{equation}
```
Here we got only one sample
$\underline{x} = \{ x_{i1} \dots x_{iM} \}_{i =1}^{N}$ and we want to
test if this sample came from the reference distribution $F$. The
distributions are still multivariate and we can use a classifiers to
reduce a multivariate goodness of fit test into a univariate one. The
...
\end{equation}
```
To perform the above test we can use kolmogorov-Smirnov test and to get
out the distribution of the kolmogorov statistics under the null
hypothesis we can set up a Monte Carlo simulation as the following:
...
statistics is greater than the threshold.
...
```{r alphasss , fig.height = 4, fig.width = 9, fig.align = "center", echo = FALSE}
par(mfrow=c(1,2))
load("data/alpha_info_1.RData")
t <- proportions(table(data[,1]))
barplot(t, col = c("#d3305d" , "#ABCDEF") , main = TeX(r"(Proportion of times we accept/reject\n $H_{0}$ when is True)" , bold = T) ,cex.main = .8, names.arg = c("Reject" , "Accept"), ylim = c(0,1) , las = 1)


hist(data[,2],freq = F , breaks = 6 , border = "white",
      col = "#B6FCD5", main = TeX(r"(p-value distribution under $H_{0}$ )", bold = T) ,cex.main = .8, xlab = TeX(r"(\hat{p}-value)"), las=1)
points(seq(0,1,length.out = 300) , rep(1,300) , col = "#C6E2FF" , cex = .8, )

legend("topright" , legend = "Uniform\ndistribution" , border = "white" , lty = 1, col = "#C6E2FF" , bty = "n", cex = .8, lwd = 2)
...
In the simulation above we are in a scenario were $n_{0} = n{1} = 70$,
the dimensionality $k = 5$ and the simulation size $P = 100$ and we are
dealing with goodness of fit test. First plot show the empirical size
...
Increasing $P$ it would fit better.

The real goal of this type of simulations is to see how an algorithm
...
{plot(n0s ,alpha_Friedmann , type = "l" , ylim = c(0,0.2) , lwd = 2 , ylab = TeX(r"($\alpha$)")  ,xlab = TeX(r"(sample size $n_{0}$)") , col = "#2274A5" , main = TeX(r"($alpha$ vs $n_{0}$ with $k = 5$ and $n_{1} = 70$)", bold = T), las=1)
...
{plot(alpha_fr_change_k, type = "l" , ylim = c(0,0.2) , lwd = 2 , ylab = TeX(r"($\alpha$)")  ,xlab = TeX(r"(dimensionality $k$)") , col = "#2274A5" , main = TeX(r"($alpha$ vs $k$ with $n_{0} = 70$ and $n_{1} = 70$)", bold = T), las=1)
points(alpha_per_change_k, type = "l" , col = "#E7EB90" , lwd = 2)
...
are close each other is more difficult to discover the difference and
reject $H_{0}$ then.

In the simulation study we choose Gaussian parametric family for both
$F_{x}$ and $F_{z}$, where $F_{z}$ has diffenent position parametes
$\underline{\mu}$. We chose closed-form of Wasserstein distance as a
distance $D(F_{x},F_{z}) = D(\underline{\mu_{x}} ,\underline{\mu_{z}})$
. $$
...
plot(distance,powers, main = TeX(r"(Power vs Distance $W_{2}^{2}(X,Z)$)" , bold = T), xlab = "distance", ylab = "power",
     col = 'darkblue' , type = "o" , lwd = 2, las=1, cex.main=main_dimesion)
...
### Two sample hypothesis test power vs sample size and power vs dimensionality
We focus our studies into the context of two sample hypothesis test.
We set in differents scenarios, where the based on our knoledge simulation, the sample $\underline{x}$ came from a $F_{1} \sim N(\underline{\mu_{1}},\Sigma)$ and the sample $\underline{z}$ came from a $F_{2} \sim N(\underline{\mu_{2}},\Sigma)$ whith different $\underline{\mu_{2}}$. Greater the distance from  $\underline{\mu_{1}}$ and $\underline{\mu_{2}}$, more separeted will be the two distribution we are comparing, and so in all likelihood the test can detect the differences better.
Specifically,we tweak the parameter $n_{0}$ and $k$ to get information about how the power varies with the distance and with the parameters values.

The plot on the left show how the power change accordinf to the distance, let varying the sample size $n_{0}$.

The plot on the right  show how the power change accordinf to the distance, let varying the dimensionality $k$.
...
colors <- c('darkblue', 'darkred', 'darkorchid2', 'darkorange')


plot(1,1, type='n', xlim=c(0,3), ylim=c(0,1), xlab='distance',ylab='power', las=1, main = TeX('Power over distance with different $n_0$ with $n_1$=70 & k=20', bold=T), cex.main=main_dimesion) 
for(i in 1:length(distance_power_n0)){
  points(distance_power_n0[[i]], type='l', col=colors[i], lwd=2)
  }

legend('bottomright', legend=c(20,50,70,100), lwd=3, col = colors, bty='n', title = 'n0') 



load('data/distance_power_k.RData')

plot(1,1, type='n', xlim=c(0,4), ylim=c(0,1), xlab='distance',ylab='power', las=1, main=TeX('Power over distance with different k with $n_0$=70 & $n_1$=60', bold=T),cex.main=main_dimesion)
for(i in 1:4){
  points(distance_power_k[[i]], type='l', col=colors[i], lwd=2)
  }

legend('right', legend=c(5, 20, 50, 100), lwd=3, col = colors, bty='n',title = 'k')

```

## Comments
In both plots, greater the distance, gretear the power will be.
Increasing the values of the sample size $n_{0}$, would increase the power of the test. Adding information would give more possibilities to the test in order to detect the difference between the two distribution and the performance of the test both on the side and the power would be better.
Increase the dimesionality will increase the distance from the distributions first, with the small amount of data,  higher the dimension , the thinner the information effectively available to resolve the difference would be.
...
The dataset consists a of structural and functional fMRI data from over
85 individuals with ASD and 93 TD controls.

Since data were taken in different labs, it's worth removing the
lab-specific effect normalizing over all the differents labs.

As the previus study suggests, to try to separate the two groups (ASD vs TD) we will by digging (linear) dependencies of cortical regions. Such that we will study the time series of the autocorrelation instead of the original ones.
The autocorrelation is defined as : $AC(t1,t2) = Corr(X_{t_1} ,X_{t_2})$.
...
### Perform the test with td (label as 0) and td (label as 1) subjects
...
```{r plot score_same, echo=FALSE , warning= FALSE}
...
### Comments
The first plot show the two distrubution of the Kolmogorov-Smirnov scores for both the sample. The second one show the distribution of the statistics under the null hypothesis. The red line is the quantile at level $1- \alpha$ $q_{\alpha}$  of the distribution, and the blue line is the observed value of the statistic we get with our data.

Since the decision rule is to reject the null hypothesis if the observed value of kolmogorov smirnov statistics is greater than $q_{\alpha}$, we reject the null hypothesis. Data don't show enough difference in order to reject the null hypothesis, we claim that the distribution of the scores are the same.
...
Since the decision rule is to reject the null hypothesis if the observed value of kolmogorov smirnov statistics is greater than $q_{\alpha}$, we reject the null hypothesis. Data show enough difference in order to reject the null hypothesis, we claim that the distribution of the scores of asd and td subjects are differents.

### Conclusion and final remark

In the case of intersest we are dealing with a noisy and small set of data, and the higher the dimension the "thinner" the information effectively available to 
