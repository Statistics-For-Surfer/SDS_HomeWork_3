...
plot(1,1, type='n', xlim=c(0,3), ylim=c(0,1), xlab='distance', ylab='power', las=1, main='Power over distance with different n0 \n n1=70 & k=20')
for(i in 1:length(distance_power_n0)){
  points(distance_power_n0[[i]], type='l', col=colors[i], lwd=3)
}
...
plot(1,1, type='n', xlim=c(0,8), ylim=c(0,1), xlab='distance', ylab='power', las=1, main='Power over distance with different k \n n0=70 & n1=60')
for(i in 1:length(distance_power_k)){
  points(distance_power_k[[i]], type='l', col=colors[i], lwd=3)
}
...
The dataset consists a of structural and functional fMRI data from over 85 individuals with ASD and 93 TD controls.

Since data were taken in different labs, it's worth removing the lab-specific effect.

```{r scale dataset by lab}
...
```{r split train dataset , warning = FALSE}
...
```{r performing the Friedmann procedure same dataset, warning = FALSE}
### Friedmann procedure
x_data <- td_data
...
```{r performing the Friedmann procedure different dataset, warning = FALSE}
### Friedmann procedure
P <- 100
k <- dim(train_x_data)[2] - 1
n0 <-  dim(train_x_data)[1]
