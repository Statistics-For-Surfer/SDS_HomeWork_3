...
Friedman_procedure <- function(P){
  ...
  for(i in 1:P){
    ...
    u_p <- rbind(x, z_p)
    glm_model <-glm(label ~ ., data = u_p,maxit = 100)
    x_scores <- predict(glm_model , x[,1:k])
    z_scores <- predict(glm_model,z[,1:k])
...
data <- alpha_info(P = P, percentile_kolm = permutation_quantile)
...
```{r p_values}
hist(data[,2] , freq = F , main = "empirical p-values distribution", col = "cyan4")
