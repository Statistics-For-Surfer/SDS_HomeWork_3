...
Since the data we collect are a finite sample ($n_{0} + n_{1} = 93 + 85 = 178$) with high dimensionality $k = 116 \times 3 = 348$ the machine learning algorithm as logistic regression would not work. Such that we need to regularize setting an early stopping and a $l_{2}$ regularization.
...
plot(first, xlim=c(0,1), xlab='scores', lwd=.1, las=1, main= "Distribution of scored for two group of TD subjects")
lines(second, lwd=.1)
...
legend('topright', legend=c('group 1', 'group 2'), pch = 15, col=dens_col, cex=1.2, bty='n')
grid(lty=2, lwd=.6)
...
#### Different groups (TD and ASD subjects)
...
plot(second, xlim=c(0,1), xlab='scores', lwd=.1, las=1, main='Distribution of scored for TD and ASD subjects')
lines(first, lwd=.1)
