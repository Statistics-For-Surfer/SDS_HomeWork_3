...
alpha_info <- function(P, percentile_kolm){
  ...
  for(i in 1:P){
    ...
    p_values[i] <- ks.test(x_scores, z_scores, alternative = "two.sided",simulate.p.value= T	
)$p.value
   
  }
  
  data = as.data.frame(cbind(prop_rej,p_values))
  colnames(data) <- c("KS", "pvalues")
