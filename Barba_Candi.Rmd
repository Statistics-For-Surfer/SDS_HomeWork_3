...
rm(list = ls())
#load packages
packs <- c('MASS', 'pracma', 'psych','caTools','e1071','dgof','latex2exp', 'caret', 'glmnet')
...
in the real case we simply got two sample without having this
info..[TODO] continua
...
```
[TODO] Comment plot above (in caso a destra rimuovi k = 100 iterando la i fino a 3 e togliendo l'etichetta dalla legenda)
...
[TODO] Why we use time series of autocorrelations. 
...
We take means, medians and standard deviations as summaries of our data.
...
n1 <- dim(test_x_data)[1]
u_same <- as.matrix(rbind(train_x_data,test_x_data))

kk <- Friedman_procedure(P, x_data = train_x_data, z_data = test_x_data, permut = T)
...
scores <- predict(model_obs, u_same[ , 1:k])

  
k_obs <- ks.test(scores[1:n0], scores[(n0+1):(n0+n1)])$statistic
k_obs < quantile(kk, 0.95)

hist(scores[1:n0], freq = F, xlim = c(-2,2), ylim = c(0,4))
hist(scores[n0:(n0+n1)], add = T, col = "green", freq = F)
hist(kk)
abline(v = quantile(kk, 0.95))
k_obs < quantile(kk, 0.95)

```

```{r plot score_same, echo=FALSE}
...
n0 <-  dim(train_x_data)[1]
n1 <- dim(z_data)[1]
u_diff <- as.matrix(rbind(train_x_data,z_data))
kk <- Friedman_procedure(P,x_data = train_x_data, z_data = z_data ,permut = T , a = 1 , m = 100000)
...
scores <- predict(model_obs ,u_diff[,1:k])
k_obs <- ks.test(scores[1:n0],scores[(n0+1):(n0+n1)])$statistic
k_obs < quantile(kk , 0.95)

model_obs <- glm(label~., data = u_diff) 
scores <- predict(model_obs ,u_diff[,1:k])
k_obs <- ks.test(scores[1:n0],scores[(n0+1):(n0+n1)])$statistic

k_obs < quantile(kk , 0.95)
sum(scores < 0)
#hist(scores[1:n0],freq = F)
#hist(scores[n0:(n0+n1)],add = T , col = "green",freq = F)
