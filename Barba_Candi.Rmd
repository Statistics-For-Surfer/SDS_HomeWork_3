...
### Friedmann procedure
P <- 100
k <- dim(train_x_data)[2] 
n0 <-  dim(train_x_data)[1]
n1 <- dim(test_x_data)[1]
u_same <- as.data.frame(rbind(train_x_data,test_x_data))

kk <- Friedman_procedure(P,x_data = train_x_data, z_data = test_x_data ,permut = T)

model_obs <- glm(label~., data = u_same)
scores <- predict(model_obs ,u_same[,1:dim(u_same)[2]])
k_obs <- ks.test(scores[1:n0],scores[n0:(n0+n1)])$statistic
...
u_diff <- as.data.frame(rbind(train_x_data,z_data))
kk <- Friedman_procedure(P,x_data = train_x_data, z_data = z_data ,permut = T)

model_obs <- glm(label~., data = u_diff , maxit = 100)
