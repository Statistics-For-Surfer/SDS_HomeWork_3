...
```{r meta test, warning = FALSE}


meta_test <- function(P,x,z){
  u_obs <- rbind(x,z)
  labels <- c(rep(0,n0),rep(1,n1))
  meta_kolm <- rep(NA, P)
  for(i in 1:P){
    idx <- sample(x = 1:(n0+n1) , size = n0+n1)
    z_p <- Take_sample_normal(n1,mu = mu, sigma = sigma , label = 1)
    u <- rbind(x,z_p)
    u$label <- labels[idx]
    glm_model <- glm(label ~ ., data = u)
    scores <- predict(glm_model , u_obs[,1:k])
    meta_kolm[i] <- ks.test(scores[u$label == 0],scores[u$label == 1])$statistic
  }
  return(meta_kolm)
}

x <- Take_sample_normal(n0,mu = mu , sigma = sigma, label = 0)
z <- Take_sample_normal(n1,mu = mu , sigma = sigma, label = 1)
meta_k <- meta_test(P = P , x = x , z = z)
...
```{r pps}
sum(data$p > 0.05 )/ length(data$p)
hist(data$p,breaks = 8)
```
...
k <- 10
P <- 100
alpha_permutation <- rep(NA ,length(n0s) )
...
for(i in 1:length(n0s)){
  ...
  alpha_permutation[i] <- 1-mean(alpha_info(P, percentile_kolm = quantile(per_kolm , 1 - alpha))$KS)
  alpha_Friedmann[i] <- 1-mean(alpha_info(P, percentile_kolm = quantile(p_kolm , 1 - alpha))$KS)
  }
