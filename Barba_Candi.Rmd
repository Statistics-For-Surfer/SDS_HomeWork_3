...
Friedman_procedure <- function(P,x_data , z_data , permut = FALSE, a = 1 , m = 10){
...
kk <- Friedman_procedure(P,x_data = train_x_data, z_data = test_x_data,permut = T , a = 1 , m = 100000)
...
scores <- predict(model_obs ,u_same[,1:k] , s = 0.05)
...
?predict
scores <- predict(model_obs ,u_diff[,1:k] , s = 0.13)
k_obs <- ks.test(scores[1:n0],scores[(n0+1):(n0+n1)])$statistic
k_obs < quantile(kk , 0.95)
